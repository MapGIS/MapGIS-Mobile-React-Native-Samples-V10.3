...
import { View , Text, TouchableOpacity, StyleSheet, Dimensions,DeviceEventEmitter} from "react-native";
...
export default class MapGraphicInterActive extends Component {
    ...
    componentDidMount() {
            ...
            switch(this.state.drawType)
            {
                ...
                case 1://圆
                    ...
                    if (this.state.circleDotCount == 1) {                        
                        ...
                        await graphicCircle.setCenterAndRadius(this.state.points[0], radius);
                    ...
                    case 3: //虚线
                        ...
                        else if(this.state.lineDotCnt == 2)
                        {
                            ...
                            this.graphicStippleLine = await graphicStippleLineModule.createObj(this.state.points[0], this.state.points[1]);               
                            await this.graphicStippleLine.setColor("rgba(100, 200, 0, 180)");
                            await this.graphicStippleLine.setLineWidth(15);                        
                            await this.stippleLineGraphicsOverlay.addGraphic(this.graphicStippleLine);   
                        }
                        await this.mapView.refresh();                        
                        break;
                    case 4://纹理线
                        ...
                        if (this.state.isFirstPoint) {
                            ...
                            let image = new Image();
                            let bitmap1 = await image.createObjByLocalPath(IMG_FILE_PATH);
                            ...
                            await this.texturelinGraphicsOverlay.addGraphic(this.graphicTextureLin);                            
                            this.setState({isFirstPoint:false});                        
                        } else {                            
                            await this.graphicTextureLin.appendPoint(dot);                           
                        }
                    ...
                    case 5://多边形
                        ...
                        {                                       
                            ...
                            if (this.state.dotLst.length > 2) {
                                this.setState({dotLst:this.state.dotLst.concat([this.state.dotLst[0]])});       // 上层必须构建闭合的区  
                            ...
                            if (this.state.dotLst.length > 3) {
                                ...
                                this.setState({dotLst:this.state.dotLst.filter((_, i) => i !== this.state.dotLst.length-1)                                });  
                            }
                        }               
                        await this.mapView.refresh();
                        break;
       ...
       DeviceEventEmitter.addListener("com.mapgis.RN.Mapview.LoadMapListener_Start", async res => {
       
       });

       DeviceEventEmitter.addListener("com.mapgis.RN.Mapview.LoadMapListener_Fail", async res => {
    ...
    checkGraphicHasFinished = async() =>{
        ...
        if (this.state.circleDotCount == 1) {
            await this.mapView.getGraphicsOverlay().removeGraphic(await this.mapView.getGraphicsOverlay().getGraphicCount() - 1);
        }
        //保证圆图形正确重新绘制
        this.setState({circleDotCount: 0 });
        ...
        if (this.graphicPolygon != null) {
            if (this.state.dotLst.length == 1) {
                await this.mapView.getGraphicsOverlay().removeGraphic(await this.mapView.getGraphicsOverlay().getGraphicCount() - 1);
                await this.polygonGraphicsOverlay.removeGraphic(await this.polygonGraphicsOverlay.getGraphicCount() - 1);
        ...
        //线、折线是否绘制完成处理
        if (this.graphicPolylin != null && await this.graphicPolylin.getPointCount() == 1) {
            await this.mapView.getGraphicsOverlay().removeGraphic(await this.mapView.getGraphicsOverlay().getGraphicCount() - 1);
            this.graphicPolylin = null;
        }
        if (this.graphicTextureLin != null && await this.graphicTextureLin.getPointCount() == 1) {
            await this.mapView.getGraphicsOverlay().removeGraphic(await this.mapView.getGraphicsOverlay().getGraphicCount() - 1);
