...
import {MGMapView,SketchEditor,SketchDataType,MeasureType,CoordinateType, CoordinateConvert, CoordinateConvertParameter} from "@mapgis/mobile-react-native";
...
export default class MapGesturesListen extends Component {
    ...
    componentDidMount() {
        DeviceEventEmitter.addListener("com.mapgis.RN.SketchEditor.geometry_changed", async e => {

        });
        DeviceEventEmitter.addListener("com.mapgis.RN.SketchEditor.vertex_selected", async e => {
     
        });
        DeviceEventEmitter.addListener("com.mapgis.RN.SketchEditor.undo_state_changed", async e => {
           alert(e.undoResult);
        });
        DeviceEventEmitter.addListener("com.mapgis.RN.SketchEditor.redo_state_changed", async e => {
         alert(e.redoResult);
        });

        
        DeviceEventEmitter.addListener("com.mapgis.RN.SketchEditor.sketch_state_changed", (res0) => {
            this.setState({
                // logs: [
                //     {
