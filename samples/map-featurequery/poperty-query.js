...
import {View, ToastAndroid, TouchableOpacity, Text,Button, StyleSheet, TextInput, DeviceEventEmitter,FlatList} from "react-native";
...
export default class MapPopertyQuery extends Component {
    ...
    _featureQuery = async () => {
        ...
        if(vectorLayer != null)
        {
            ...
            for (var j = 0; j < featureLst.length; j++) {
                ...
                let attributes = await feature.getAttributes();
                 ...
                 if (IsExistName == true) {  
                    ...
                    //结果列表数组
                    this.setState({qryresult:this.state.qryresult.concat([featureName])});                 
                }
                //获取要素的几何信息（默认查询点要素）
            ...
            ToastAndroid.show('查询结果总数为：'+getTotalFeatureCount, ToastAndroid.SHORT);
        }
    };
    ...
    renderItem = (item) => (
        <View style={style.item}>
            {<Text style={style.txt}>{item.item}</Text>}
        </View>
    );
    ...
    render() { 
        return (
            <View style={styles.container}>
                ...
                />
                <FlatList style={style.resultView} data={this.state.qryresult} renderItem={this.renderItem} 
